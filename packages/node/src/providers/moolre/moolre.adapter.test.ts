...
import { MoolrePaymentDTO } from '../../core/provider-schemas/moolre.schema.js';
...
describe('MoolreAdapter', () => {
  ...
  describe('initiatePayment', () => {
    const validPayload: MoolrePaymentDTO = {
      amount: 100,
    ...
    it('should throw VoltaxValidationError if reference is missing', async () => {
      const payload = { ...validPayload, reference: '' };
      await expect(adapter.initiatePayment(payload)).rejects.toThrow();
    });

    it('should throw VoltaxValidationError if callbackUrl is missing', async () => {
      const payload = { ...validPayload, callbackUrl: '' };
      await expect(adapter.initiatePayment(payload)).rejects.toThrow();
    });

    it('should throw VoltaxValidationError if redirectUrl is missing', async () => {
      const payload = { ...validPayload, redirectUrl: '' };
      await expect(adapter.initiatePayment(payload)).rejects.toThrow();
    });
    ...
    it('should initialize payment successfully', async () => {
      ...
      expect(mockPost).toHaveBeenCalledWith('/embed/link', {
        ...
        redirect: validPayload.redirectUrl,
        reusable: '0',
