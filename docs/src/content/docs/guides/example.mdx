...
// Initialize Voltax with a single provider
...
```typescript
import { VoltaxAdapter, Currency, PaymentStatus } from '@noelzappy/voltax';
...
async function initiatePayment(request: PaymentRequest) {
  ...
  switch (provider) {
    case 'paystack':
      return voltax.paystack.initiatePayment(payload);

    case 'flutterwave':
      return voltax.flutterwave.initiatePayment(payload);

    case 'hubtel':
      return voltax.hubtel.initiatePayment({
        ...payload,
...
export async function GET(
  ...
  try {
    const result = await paystack.verifyTransaction(params.reference);
