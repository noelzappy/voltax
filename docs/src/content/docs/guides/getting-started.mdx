...
<Tabs>
  <TabItem label="npm">```bash npm install voltax-node ```</TabItem>
  <TabItem label="pnpm">```bash pnpm add voltax-node ```</TabItem>
  <TabItem label="yarn">```bash yarn add voltax-node ```</TabItem>
</Tabs>
